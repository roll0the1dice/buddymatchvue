<template>
  <div v-if="userList">
    <van-card v-for="user in userList" :desc="user.personalBio" :title="user.displayName" :thumb="user.avatar">
      <template #tags>
        <van-tag plain type="danger" v-for="tag in tags"><span>{{ tag }}</span></van-tag>
...
onMounted(async () => {
  ...
  if (tags.length > 0) {
    try {
      const res = await buddyUserControllerApi.searchUserByTags(tags);
      const { data, statusCodeValue }: ApiResponseListBuddyUser = res.data;
      if (statusCodeValue === 200 && data) {
        userList.value = data;
        var result = data.map((user: BuddyUser) => {
          console.log(JSON.parse(user.tagName as string));
          const _tagNameList = JSON.parse(user.tagName as string);
          return _tagNameList;
        });
